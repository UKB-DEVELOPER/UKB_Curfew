...
Staus = function ( errNum , data)
    ...
    if ( errNum == 200 and data == scriptId and ResourceName == 'Ry_Curfew') then
        ...
        print("^2##   ^5https://fivem.unknowkubbrother.net  ^2##") 
        print("^2###########################################") 
        Status = true
        ...
        print("^1##   ^5https://fivem.unknowkubbrother.net  ^1##") 
        print("^1###########################################") 
...
Connect = function ()
    ...
    end, "POST", json.encode(Ry), { ["Content-Type"] = "application/json" })
...
AddEventHandler(ResourceName..':sentlocation', function(x,y,z,type, title)
	id = id+1
	zone[id] = {x = x, y = y, z = z, type = type, title = title, id=id}
